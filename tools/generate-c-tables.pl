...
# CC -> storage divisor, from the sysex offset table
my %divisor;
for my $o ( values %{$tables->{sysex}->{offsets}} )
{
	$divisor{$o->{cc}} = $o->{divisor} if defined $o->{divisor};
}
...
for my $id ( @ids )
{
	...
	$c .= sprintf "\t\t.cc = %s, .lsb_cc = %s, .bits = %d,\n",
		...
		( $e->{bits} || 7 );
	# unsigned 14-bit parameters are put on the wire by their storage divisor
	my $wire_div = ( ($e->{bits}||7) == 14 && defined $e->{min} && $e->{min} >= 0
	                 && defined $divisor{$id} ) ? $divisor{$id} : 0;
	$c .= sprintf "\t\t.min = %.17g, .max = %.17g, .digits = %d, .format = %s,\n",
		( defined $e->{min} ? $e->{min} : 0 ),
		( defined $e->{max} ? $e->{max} : 0 ),
		( defined $e->{dig} ? $e->{dig} : 0 ),
		cstr($e->{format});
	$c .= sprintf "\t\t.wire_divisor = %.17g,\n", $wire_div;
